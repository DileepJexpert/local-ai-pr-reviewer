...
On macOS, the reviewer now also displays a clear **“ACTION REQUIRED”** dialog before IDFC Coder opens. Click **Open IDFC Coder**, then follow the displayed steps: click IDFC Coder, press `Cmd + V`, and press Enter once. If IDFC Coder displays a plan, press `Shift + Tab` to approve it, type `Proceed`, and press Enter. Wait for `REVIEW COMPLETE` in Terminal.
